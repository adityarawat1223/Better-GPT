...
void ModelScript() {
    ...
    std::string js = R"((async () => {
        try {
            ...
            const response = await fetch(`https://chatgpt.com/backend-api/models?iim=false&is_gizmo=false`, {
                method: 'GET',
                headers: {
            ...
            console.log("[Models Info] Status:", response.status);
        } catch (err) {
            console.error("[Models Info]", err);
        }
    })();)";
