...
void ChatTextParser::Chat_Text_Parser()
{
    ...
    try {
        ...
        AppState::UpdateChatStatus(chat_id, Status::Parsing);

        if (!initjson.contains("mapping")) {
        ...
        if (!initjson.contains("current_node") ||
            !initjson["current_node"].is_string()) {
            AppState::UpdateChatStatus(chat_id, Status::Cached);
            return;
        }
        ...
        if (!chat_id.empty()) {
            AppState::UpdateChatStatus(chat_id, Status::Error, e.what());
        }
    }
}
